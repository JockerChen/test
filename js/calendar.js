...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    showHoverInfo: function (obj) { // hover 时显示当天信息
      ...
      $(this.opts.startOrEnd).val(changeStr);
    ...
    renderDOM: function () { // 渲染DOM
      this.$calendar_zzc = $('<div class="zzc"></div>');
      this.$calendar_box = $('<div class="calendar"></div>');
      ...
      this.$calendar_today.html(_dayStr);
        ...
        this.$calendar_box.append(this.$calendar_title, this.$calendar_week, this.$calendar_date);
        this.$calendar.append(this.$calendar_zzc, this.$calendar_box);
      // this.$calendar.show();
    },
    ...
    inital: function () { // 初始化
      ...
      this.$startOrEnd=$(this.opts.startOrEnd);
      this.$fnCall=this.opts.fnCallback;
      console.log(typeof this.$fnCall);
      this.showCalendar();
      ...
      this.$calendarDate_item.bind('click', function () {
        console.log(typeof that.$fnCall);
        if(!!that.$fnCall){that.$fnCall.call();}
        $("#calendar").empty().hide();
      });
    },
